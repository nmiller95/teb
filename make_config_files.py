...
def make_flux_ratio_priors(filename):
    ...
    with open(f'config/{filename}_flux_ratio_priors.yaml', 'w') as c:
        ...
        # c.write('# flux_ratio is only required if performing a fit using synthetic flux ratios in visual bands.\n')
        # c.write('# This is not recommended for normal use of teb.')
        c.write('\n')
